...
## Ported onto main after the supatree work (2026-09-23)

The branch sat open while main grew review trees, a status dashboard, a
watcher, and a per-branch fetch path of its own (`pkg/supatree/prfetch.go`,
`github.ResolvePR`, `Cache.MarkUnreachable`). Porting it forced four decisions:

- **Review and checks.** Main's `PRInfo` gained `Review`, `Checks` and
  `HeadOID`, which the dashboard, `status` and the check-failure events read.
  The REST listing has the head SHA but neither verdict, and a check run
  finishing does not change the listing's ETag. So polls carry both over from
  the previous entry for the same PR, and `refreshDetails` re-reads them with
  one GraphQL `gh pr view` per *open* PR — on the `MaxAge` schedule, or at once
  when a poll saw the PR move (`PRInfo.DetailedAt`). That is the one recurring
  GraphQL cost left; merged, closed and PR-less branches never pay it.
- **Review members.** Keyed `pr:<repo>#<n>` and sat on a tree-local branch, so
  `Target` gained `Key` and `Ref`: a pinned target is matched by number on the
  polled page, and its fallback is a by-number lookup.
- **Renamed branches.** Main's rename semantics (keep the number, zero
  `FetchedAt`) are kept. A poll matches by head, then by the cached number; a
  fallback runs main's head-then-number `resolvePR`; and an entry never
  verified under its key is looked up rather than "confirmed" by a poll that
  lacks it.
- **One backoff for invisible repos.** Main's per-key `MarkUnreachable` (6h)
  and this branch's per-repo `Unavailable` (until forced) did the same job.
  They became `RepoState.UnavailableUntil`: per repo, 6h, bypassed by a forced
  refresh, and applied to non-GitHub remotes by checkout path.

`supatree.FetchTargets`/`FetchPRs` stay as supatree's entry point so the
sidebar, dashboard, watcher and `status --refresh` did not each grow their own
`Sync` call.
