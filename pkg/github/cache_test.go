...
func TestCacheRenameKeepsNumberButForcesRefetch(t *testing.T) {
	...
	mustMutate(t, c, func(w *Writable) { w.Rename("st/old-slug/ads", "st/new-slug/ads") })
	...
	if !info.DetailedAt.IsZero() {
		t.Error("renamed entry's review/checks must be re-read too")
	}
}
...
func TestCacheRenameTerminalEntryIsStale(t *testing.T) {
	...
	c := newTestCache(t)
	mustMutate(t, c, func(w *Writable) {
		w.Set("old", &PRInfo{Number: 1, Status: PRMerged, FetchedAt: time.Now()})
		w.Rename("old", "new")
	})
...
func TestCacheRenameMissingEntry(t *testing.T) {
	c := newTestCache(t)
	mustMutate(t, c, func(w *Writable) { w.Rename("absent", "new") })
	if c.Get("new") != nil {
...
func TestCacheRef(t *testing.T) {
	c := newTestCache(t)
	mustMutate(t, c, func(w *Writable) {
		w.Set("has-pr", &PRInfo{Number: 7, Status: PROpen, URL: "https://github.com/org/repo/pull/7"})
		w.Set("no-pr", &PRInfo{Status: PRNone})
	})
...
func TestRepoStateUnavailableExpires(t *testing.T) {
	now := time.Now()
	s := RepoState{UnavailableUntil: now.Add(UnreachableBackoff)}
	if !s.Unavailable(now) {
		t.Error("a repo inside its backoff window should be unavailable")
	}
	if s.Unavailable(now.Add(UnreachableBackoff + time.Minute)) {
		t.Error("an expired backoff should let the repo be tried again")
	}
	if (RepoState{}).Unavailable(now) {
		t.Error("a repo never marked should be available")
	}
}
